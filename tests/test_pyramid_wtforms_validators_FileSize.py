...
class TestPyramidWTFormsValidatorsFileSize(unittest.TestCase):
    ...
    def test_should_pass_if_file_size_is_higer_or_equal_than_lower_limit(self):
        # mimic single file
        self.field.data = FieldStorage(cgiFieldStorage())
        self.field.data.filename = 'foo.txt'
        test_file_data = b'foo'
        self.field.data.file = io.BytesIO(test_file_data)
        self.assertIsNone(FileSize(min=len(test_file_data)-1)(self.form, self.field))
        # boundary test
        self.assertIsNone(FileSize(min=len(test_file_data))(self.form, self.field))
        ...
        self.assertIsNone(FileSize(min=len(test_file_data1)-1)(self.form, self.field))
        # boundary test
        self.assertIsNone(FileSize(min=len(test_file_data1))(self.form, self.field))
        
    def test_should_raise_error_if_file_size_is_not_higer_or_equal_than_lower_limit(self):
        # mimic single file
        self.field.data = FieldStorage(cgiFieldStorage())
        self.field.data.filename = 'foo.txt'
        test_file_data = b'foo'
    ...
    def test_should_pass_if_file_size_is_lower_or_equal_than_upper_limit(self):
        # mimic single file
        self.field.data = FieldStorage(cgiFieldStorage())
        self.field.data.filename = 'foo.txt'
        test_file_data = b'foo'
        self.field.data.file = io.BytesIO(test_file_data)
        self.assertIsNone(FileSize(max=len(test_file_data)+1)(self.form, self.field))
        # boundary test
        self.assertIsNone(FileSize(max=len(test_file_data))(self.form, self.field))
        ...
        test_file_data2 = b'bar123'
        self.field.data[1].file = io.BytesIO(test_file_data2)
        self.assertIsNone(FileSize(max=len(test_file_data2)+1)(self.form, self.field))
        # boundary test
        self.assertIsNone(FileSize(max=len(test_file_data2))(self.form, self.field))
        
    def test_should_raise_error_if_file_size_is_not_lower_or_equal_than_upper_limit(self):
        # mimic single file
        self.field.data = FieldStorage(cgiFieldStorage())
        self.field.data.filename = 'foo.txt'
        test_file_data = b'foo'
